...
export default function Home() {
  ...
  const handleBatchInterrogate = async () => {
    if (images.length === 0) {
    ...
    for (let i = 0; i < images.length; i++) {
      try {
        const response = await fetch("/api/gpt-interrogate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            image: images[i].content,
            apiKey: openaiApiKey,
            currentCaption: images[i].caption,
            ...gptOptions,
          }),
        });

        if (!response.ok) throw new Error("Failed to interrogate image");

        const { caption } = await response.json();

        setImages((prevImages) =>
          prevImages.map((img, index) =>
            index === i ? { ...img, caption } : img
          )
        );
    ...
    setIsBatchProcessing(false);
    toast.success("Batch interrogation completed");
  };
    ...
    <section className="flex flex-col items-center justify-center p-4">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          {selectedIndex !== null ? (
              ...
              <CardBody className="flex flex-col p-4">
                ...
                <div className="mt-4">
                  <Button
                    fullWidth
                    startContent={<LuBrainCircuit />}
                    onClick={handleBatchInterrogate}
            ...
            <p className="text-center">
              No image selected. Upload an image or select one from the sidebar.
            </p>
          )}
        </div>
        </div>
      ...
      {isBatchProcessing && (
        <div className="fixed bottom-4 right-4 bg-background p-4 rounded-lg shadow-lg">
          <p>Processing: {batchProgress.toFixed(2)}%</p>
        </div>
      )}
    </section>
  );
